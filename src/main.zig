...
const maxSpeed = 8;
const jumpVec = rl.Vector2.init(0, -6);
...
const Bird = struct {
    ...
    fn getCollisionBox(self: Bird) rl.Rectangle{
        return rl.Rectangle.init(self.position.x, self.position.y, 80, 80);
    }
};
...
const GameState = struct {
    ...
    pub fn getArena(self: GameState) error{OutOfMemory}! *const std.mem.Allocator{
        if (self.arenaAlloc) |_arenaAlloc| return _arenaAlloc
        else return error.OutOfMemory;
    }

    fn init(allocator: std.mem.Allocator, rand: std.Random, pipes: *std.ArrayList(Pipe), bird: *Bird) GameState {
        return .{
            .allocator = allocator,
            .rand = rand,
...
pub fn main() !void {
    ...
    var bird = Bird {
        ...
        .position = rl.Vector2.init(64, screenHeight/2)
    };

    var state = GameState.init(allocator, rand, &pipes, &bird);
...
fn makePipeCollisions(pipes: *[]Pipe, arena: *const std.mem.Allocator) ?*[]rl.Rectangle {
    ...
    var pipeCollisions = 
        arena.alloc(rl.Rectangle, pipes.len)
        catch @panic("Couldn't allocate memory.");

    const height: f32 = @floatFromInt(rl.getScreenHeight());

    for (pipes.*, 0..) |pipe, idx| {
        pipeCollisions[idx] = (
            if (pipe.up) rl.Rectangle.init(
                pipe.position.x,
                0,
                pipeWidth,
                pipe.position.y
...
test "makePipeCollisions" {
    const pipes = [_]Pipe {
        .{ 
            .position = rl.Vector2.init(0,15),
            .up = false
        },
        .{ 
            .position = rl.Vector2.init(1,10),
            .up = false
        },
        .{ 
            .position = rl.Vector2.init(2,20),
            .up = false
        },
    };
...
fn attemptMakeNewPipe(state: *GameState) void {
    ...
    const amountBeforePipe = pipeWidth + 20;
    ...
    if ((state.distance - state.lastPipeDistance) >= amountBeforePipe) {
        ...
        ) catch @panic("cannot allocate memory");
...
fn doGameLogic(state: *GameState) !void {
    ...
    state.speed = @max(
        @as(f32, @floatFromInt(state.distance)) / 1000,
        state.speed
    );
    ...
    if (res.y < -15) { bird.speed = bird.speed.subtract(jumpVec); }
    bird.position = res;
...
fn PaintScreen(state: *GameState) !void {
    ...
    if (state.pipes.items.len > 0) {
        const pipeCollisions = 
            if (state.currentPipeCollisions) |col| col.*
            else @panic("Pipe collisions have not been made.");
    
        for (state.pipes.items, 0..) |pipe, idx| {
            rl.drawRectangleRec(
                pipeCollisions[idx],
                if (pipe.up) rl.Color.purple else rl.Color.green
            );
        }
    }

    rl.drawRectangleRec(birdCollision, if (state.bird.isDead) rl.Color.orange else rl.Color.gold);

    const str: [:0]u8 = @ptrCast(
        try std.fmt.allocPrint(
            arenaAlloc,
            "Distance: {d:.3} \nSpeed: {d:.3} \nScore: {}",
            .{state.distance, state.speed, state.score}
    ));
...
fn androidMain() callconv(.c) c_int { return std.start.callMain(); }
